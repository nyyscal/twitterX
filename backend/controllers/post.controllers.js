...
export const createPost = async(req,res) =>{
  try {
    ...
    if(img){
      const uploadedResponse = await coludinary.uploader.upload(img)
...
export const deletePost = async(req,res) =>{
  try {
    ...
    if(post.img){
      ...
      await coludinary.uploader.destroy(imgId)
    }
...
export const commentOnPost = async(req,res) =>{
  try {
    ...
    console.log("Error in commentController", error.message)
    res.status(500).json({error:"Internal Server Error!"})
  }
}
...
export const likeUnlikePost = async(req,res) =>{
  try {
    ...
    if(userLikedPost){
      ...
      res.status(200).json({message:"Post unliked sucesfully!"})
    }else{
      //Like if not liked 
